...
to explore
  if economic-capital > 0 [
  let posstible_neighbors other turtles-here with [economic-capital > 0 and not link-neighbor? myself]
  if any? posstible_neighbors [
    let friend one-of posstible_neighbors with [economic-capital > 0]
    if friend != nobody [
