...
directed-link-breed [ obligations obligation ] ;; representation of the social capital
...
to setup
  ...
  clear-output
  clear-all
  set-default-shape actors "person"
...
to go
  ...
  [
    ...
    if any? learners [
      ask learners [send-info-to-clients]
    ]
  ]
end
...
to listen-clients
  ...
  [
    ...
    [
      ...
      [ remove-learner ]
      [ ask learners with [user-id = hubnet-message-source]
...
;; when a new user logs in create a learner turtle
;; this turtle will store any state on the client
;; values of sliders, etc.
to create-new-learner
  let income-prob list (list (5) (poor-slider))  (list (10) (1 - poor-slider))
  create-learners 1
  [
    ...
    set label user-id
    let income-list rnd:weighted-one-of-list income-prob [ [p] -> last p ]
    set income first income-list
...
to send-info-to-clients ;; turtle procedure
  ...
  hubnet-send user-id "economic-capital" economic-capital
  hubnet-send user-id "income" income
end
...
to execute-move [new-heading]
  ...
  [
    hubnet-send-message user-id "Need economic capital to socialize"
  ]
  send-info-to-clients
end
...
to work
  ...
  if breed = learners [ send-info-to-clients ]
end

to connect
  ifelse economic-capital > 0 [
    let existing-link-neighbors obligation-neighbors
    let possible-neighbors other turtles in-radius 2 with [not member? self existing-link-neighbors]
    ifelse any? possible-neighbors [
      let friend one-of possible-neighbors
      ifelse random-float 1 < social-environment-trustworthiness [
        if friend != nobody [
        ...
        if breed = learners [hubnet-send-message user-id "Social connection was established" ]
      ] [
        if breed = learners [hubnet-send-message user-id "Sometimes trust is hard to establish. Try again"]
      ]
    ] [
    if breed = learners [hubnet-send-message user-id "Nobody nearby" ]
    ]
  ]   [
    if breed = learners [hubnet-send-message user-id "Need economic capital to establish social connection"]
    if breed = learners [hubnet-send-message user-id "Nobody will trust you, if you could not invest some resources"]
  ]
  if breed = learners [ send-info-to-clients ]
end
...
to explotate-connection
  ...
  ifelse any? lenders [
    ...
    hubnet-send-message user-id "you do not have any usefull connections"
  ]
end
    ...
    ask both-ends [
      ...
      if economic-capital >= amount [
        ...
        ask current-actor [
          ...
          ask my-obligation [
            set weight [weight] of my-obligation + 1
          ]
        ]
